// Projects.jsx — Cards con glass effect.
    ...
    >
      {project.badge && (
        <div style={{
          ...
          background: 'var(--magenta-dim)', border: '1px solid var(--magenta-glow)',
          ...
          <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
            {project.tags.map(tag => (
              <span key={tag} style={{
                ...
                border: '1px solid rgba(0,229,255,0.12)', padding: '0.2rem 0.55rem', borderRadius: '2px',
              }}>{tag}</span>
            ))}
          </div>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: 'var(--text-muted)' }}>{project.year}</span>
        </div>

        <h3 style={{
          fontFamily: 'var(--font-display)', fontSize: '1.1rem', fontWeight: 700,
          lineHeight: 1.3, color: 'var(--text)', marginBottom: '0.9rem',
        }}>{project.title}</h3>

        <p style={{ fontSize: '0.88rem', color: 'var(--text-muted)', lineHeight: 1.75, marginBottom: '0.6rem' }}>
          <span style={{ color: 'var(--text-dim)', fontWeight: 500 }}>problema — </span>{project.problem}
        </p>
        <p style={{ fontSize: '0.88rem', color: 'var(--text-muted)', lineHeight: 1.75, marginBottom: '1.75rem' }}>
          <span style={{ color: 'var(--text-dim)', fontWeight: 500 }}>solución — </span>{project.solution}
        </p>
...
export default function Projects() {
  ...
  return (
    <section id="proyectos" style={{ padding: 'clamp(5rem, 10vw, 9rem) 0', borderTop: '1px solid var(--border)' }}>
      <div className="container">
        ...
        >
          ...
          <h2 style={{
            ...
            fontWeight: 800, letterSpacing: '-0.03em', lineHeight: 1.1, color: 'var(--text)',
          }}>
            Trabajo real,<br />
            <span style={{ color: 'var(--cyan)', textShadow: '0 0 22px var(--cyan-glow)' }}>resultados concretos.</span>
          </h2>
        </motion.div>
